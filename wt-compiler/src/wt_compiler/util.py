...
def validate_importable_reference(reference: str) -> str:
    ...
    assert len(parts) == 2, (  # noqa: S101  # doctested invariant; AssertionError is the documented failure mode
        f"{reference} is not a valid importable reference, must be a dotted string."
    )
    assert parts[1].isidentifier(), (  # noqa: S101  # doctested invariant
        f"{parts[1]} is not a valid Python identifier, it will not be importable."
    )
    assert all(module_part.isidentifier() for module_part in parts[0].split(".")), (  # noqa: S101  # doctested invariant
